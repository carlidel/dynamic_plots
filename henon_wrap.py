...
def henon_lyapunov_multi(init_list, n_turns, epsilon, mu):
    initial_err = 1e-12
    x0 = np.concatenate(
...
def henon_lyapunov_square_error(init_list, n_turns, epsilon, mu):
    initial_err = 1e-12
    x0 = np.concatenate(
        (init_list[0], init_list[0], init_list[0], init_list[0], init_list[0]))
    px0 = np.concatenate(
...
def henon_megno_single(init_list, n_turns, epsilon, mu):
    initial_err = 1e-14
    x0 = np.concatenate((init_list[0], init_list[0]))
    px0 = np.concatenate((init_list[1], init_list[1]))
    y0 = np.concatenate((init_list[2], init_list[2]))
...
def henon_frequency_x(init_list, n_turns, epsilon, mu):
    ...
    signal = x + 1j * px
    fft = np.fft.fft(signal * np.hanning(signal.shape[0])[:, None], axis=0)
    afft = np.absolute(fft)
    
    c_idx = np.argmax(afft, axis=0)/n_turns
    c_idx[c_idx==0] = np.nan
    # l_idx = c_idx - 1
    # r_idx = c_idx + 1
    # #### correction ####
    # r_idx[l_idx == -1] += 1
    # c_idx[l_idx == -1] += 1
    # l_idx[l_idx == -1] += 1
    # l_idx[r_idx == len(fft)] -= 1
    # c_idx[r_idx == len(fft)] -= 1
    # r_idx[r_idx == len(fft)] -= 1
    # ####################
    # print(fft.shape)
    # print(c_idx.shape)
    
    # p1, p2, vp1, vp2 = select_indices(l_idx, c_idx, r_idx, afft)

    # co = np.cos(2*np.pi/n_turns)
    # si = np.sin(2*np.pi/n_turns)
    # scra1 = co**2 * (vp1 + vp2) ** 2 - 2 * vp1 * vp2 * (2 * co ** 2 - co - 1)
    # scra2 = (vp1 + vp2 * co) * (vp1 - vp2)
    # scra3 = vp1 ** 2 + vp2 ** 2 + 2 * vp1 * vp2 * co
    # scra4 = (- scra2 + vp2 * np.sqrt(scra1)) / scra3
    # assk = p1 + n_turns/2/np.pi + np.arcsin(si * scra4)
    return c_idx


def henon_frequency_y(init_list, n_turns, epsilon, mu):
    n_turns = prev_2_pow(n_turns)
    engine = hm.partial_track.generate_instance(
        init_list[0],
        init_list[1],
    ...
    _, _, x, px = engine.compute(n_turns, epsilon, mu, full_track=True)
    signal = x + 1j * px
    fft = np.fft.fft(signal * np.hanning(signal.shape[0])[:, None], axis=0)
    afft = np.absolute(fft)
...
def henon_frequency_x_diff(init_list, n_turns, epsilon, mu):
    n_turns = prev_2_pow(n_turns//2)
    engine = hm.partial_track.generate_instance(
        init_list[0],
        init_list[1],
        init_list[2],
    ...
    fft = np.fft.fft(signal * np.hanning(signal.shape[0])[:, None], axis=0)
    afft = np.absolute(fft)
    c_idx1 = np.argmax(afft, axis=0) / n_turns
    c_idx1[c_idx1==0] = np.nan

    _, _, x, px = engine.compute(n_turns, epsilon, mu, full_track=True)
    signal = x + 1j * px
    fft = np.fft.fft(signal * np.hanning(signal.shape[0])[:, None], axis=0)
    afft = np.absolute(fft)
    c_idx2 = np.argmax(afft, axis=0) / n_turns
    c_idx2[c_idx2 == 0] = np.nan
    return np.absolute(c_idx1 - c_idx2)


def henon_frequency_y_diff(init_list, n_turns, epsilon, mu):
    n_turns = prev_2_pow(n_turns)
    engine = hm.partial_track.generate_instance(
        init_list[0],
        init_list[1],
...
def henon_frequency_xy(init_list, n_turns, epsilon, mu):
    n_turns = prev_2_pow(n_turns)
    engine = hm.partial_track.generate_instance(
        init_list[0],
        init_list[1],
    ...
    signal = x + 1j * px
    fft = np.absolute(np.fft.fft(signal * np.hanning(signal.shape[0])[:, None], axis=0))
    data = np.argmax(fft, axis=0) / n_turns
    signal2 = y + 1j * py
    fft2 = np.absolute(np.fft.fft(signal2 * np.hanning(signal2.shape[0])[:, None], axis=0))
    data2 = np.argmax(fft2, axis=0) / n_turns
    return np.absolute(data - data2)
    ...
    for i in range(len(indices)):
        if np.any(np.isnan(data[:, i])):
            ...
            if cf3 > cf1:
                ...
                p2 = cf2
                nn = indices[i] - 1
            co = np.cos(2 * np.pi / len(indices))
            si = np.cos(2 * np.pi / len(indices))
            scra1 = co ** 2 * (p1 + p2) ** 2 - 2 * p1 * p2 * (2 * co ** 2 - co - 1)
            scra2 = (p1 + p2 * co) * (p1 - p2)
            scra3 = p1 ** 2 + p2 ** 2 + 2 * p1 * p2 * co
            scra4 = (-scra2 + p2 * np.sqrt(scra1)) / scra3
            assk = nn + len(indices) / 2 / np.pi * np.arcsin(si * scra4)
            values[i] = 1 - (assk - 1) / len(indices)
    return values


def henon_frequency_xy_double(init_list, n_turns, epsilon, mu):
    n_turns = prev_2_pow(n_turns)
    engine = hm.partial_track.generate_instance(
        init_list[0],
        init_list[1],
...
def henon_tune_x(init_list, n_turns, epsilon, mu):
    engine = hm.partial_track.generate_instance(
        init_list[0],
        init_list[1],
        init_list[2],
...
def henon_tune_y(init_list, n_turns, epsilon, mu):
    engine = hm.partial_track.generate_instance(
        init_list[0],
        init_list[1],
        init_list[2],
...
def henon_tune(init_list, n_turns, epsilon, mu):
    min_res_order = 3 # TODO:resonance_order
    max_res_order = 5
    tolerance = 0.005 # NOTA: distanza da risonanza
    n_turns = prev_2_pow(n_turns)
    engine = hm.partial_track.generate_instance(
        init_list[0],
        init_list[1],
    ...
    # TODO: add 2d tunes
    # TODO: iterate over all resonance values (3-5)
    for j in range(min_res_order, max_res_order + 1):
        for i in range(j + 1): # nx = 0, ny = 4; nx = 1, ny = 3
            tx = i
            ty = j - i
            mask1 = np.absolute(
...
henon_wrap.set_indicator(henon_lyapunov_multi, "lyapunov multi")
henon_wrap.set_indicator(henon_lyapunov_square_error, "lyapunov square error")
henon_wrap.set_indicator(henon_megno_single, "megno")
henon_wrap.set_indicator(henon_inverse_error, "inversion error")
#henon_wrap.set_indicator(henon_frequency_x, "Tune X")
#henon_wrap.set_indicator(henon_frequency_y, "Tune Y")
#henon_wrap.set_indicator(henon_frequency_x_diff, "Tune X diff")
#henon_wrap.set_indicator(henon_frequency_y_diff, "Tune Y diff")
#henon_wrap.set_indicator(henon_frequency_xy, "Tune XY")
henon_wrap.set_indicator(henon_frequency_xy_double, "Tune XY double")
henon_wrap.set_indicator(henon_tune, "tune")
henon_wrap.set_indicator(henon_tune_x, "tune x")
henon_wrap.set_indicator(henon_tune_y, "tune y")
